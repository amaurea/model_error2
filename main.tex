...
making it easy to miss. We investigate the common case of sub-pixel errors
...
&= \Big[\underbrace{G^{-1}N^{-1}G^{-1}}_A\Big]^{-1}\underbrace{G^{-1}N^{-1} m}_b
\end{align}
Inserting the expression for $G^{-1}$ we get
\begin{align}
...
costly noise-wise to be a practical mitigation strategy.
\begin{figure}
	\centering
...
\begin{figure}
	...
	are too costly to be realistic solutions to the power loss problem.}
...
\begin{enumerate}
	...
	\item \textbf{subpixel errors} and \textbf{gain errors} are common examples of
		model errors, with subpixel errors being almost universal. Other types of model error
...
We would like to thank Jon Sievers for first making me aware
