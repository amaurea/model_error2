...
\end{align}
Inserting the actual data behavior from equation~\ref{eq:gain-toy-1d}, we get
\begin{align}
	...
	N_f &= \begin{bmatrix}1 & 1 \\ 1 & 1\end{bmatrix} (f/f_\text{knee})^{-3} +
		\begin{bmatrix}1 & 0 \\ 0 & 1\end{bmatrix} =
			\begin{bmatrix}
				1 & \alpha_f \\ \alpha_f & 1
			\end{bmatrix} A_f \notag \\
	\alpha_f &= A_f^{-1} \quad,\quad
	A_f = 1 + (f/f_\text{knee})^{-3} \quad \Rightarrow \notag \\
	\text{TF}(f) &= \frac{(f/f_\text{knee})^{-3}}{1-\frac{2g_1g_2}{g_1^2+g_2^2}(1+(f/f_\text{knee})^{-3})}
	\label{eq:gain-tf-1d}
\end{align}
...
\begin{align}
	d_{ix} &= g_{ix} m_x + g_{ix} n_{ix} \quad,\quad \text{(actual)}
\end{align}
...
by the noise model:
\begin{align}
	d_{ix} &= m_x + g_{ix} n_{ix} \quad,\quad \text{(model)}
\end{align}
...
\begin{enumerate}
	...
	\item Similarly the covariance is Fourier-diagonal with power spectrum
		$\widetilde{N}$
\end{enumerate}
Under these assumptions it can be shown that when the number of passes is much
greater than one the Fourier transform of the maximum likelihood solution becomes
\begin{align}
	\hat{\widetilde{m}}_f &= \frac{\widetilde{N}^{-1}_f}{\widetilde{N}^{-1}_f + (\widetilde{\Gamma} \ast \widetilde{N}^{-1})_f} \widetilde{m}_f
\end{align}
where $\ast$ is the convolution operator. We see that in the limit of uncorrelated
